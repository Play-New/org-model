/**
 * Lay the model out for the map: nodes in columns by role (platform → service →
 * core → contracts, left to right, the way dependency flows), with edges for
 * supports (node → contract) and dependsOn (node → node). Pure — no react-flow
...
export interface FlowNode {
  ...
  data: { label: string; kind: 'contract' | 'node'; orientation?: string; state: FillState };
}
...
export interface FlowEdge {
  ...
  source: string;
  target: string;
  kind: 'supports' | 'depends';
}

const COL: Record<string, number> = { platform: 0, service: 1, core: 2, contract: 3 };
const COL_W = 240;
const ROW_H = 92;
...
export function buildMap(model: OrgModel): { nodes: FlowNode[]; edges: FlowEdge[] } {
  ...
  for (const n of model.nodes) {
    nodes.push({
      id: nid(n.id),
      position: place(COL[n.orientation]),
      data: { label: n.name || n.id, kind: 'node', orientation: n.orientation, state: nodeState(n) },
    });
  }
  for (const c of model.contracts) {
    nodes.push({
      ...
      data: { label: c.withParty || c.id, kind: 'contract', state: contractState(c) },
    });
  }
  ...
  for (const n of model.nodes) {
    for (const c of n.supports) {
      if (present.has(cid(c))) edges.push({ id: `s:${n.id}->${c}`, source: nid(n.id), target: cid(c), kind: 'supports' });
    }
