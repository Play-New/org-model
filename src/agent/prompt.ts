...
export function buildSystemPrompt(ctx: PromptContext): string {
  ...
  const lines = [
    ...
    '- For every contract and node write a DETAILED `note` in prose — the real substance mined from the documents: specific facts, numbers, names, mechanisms, history, nuances. Not a one-line summary. Several short paragraphs, with `##` sub-headings where they help, and inline `(source-id)` citations throughout. If the documents are rich, the note must be rich — a reader should learn the actual situation, not a label. The structured fields are just the index; the note is the full human read. No field dumps, but no thinness either.',
